...
def save_imgs(args, e1, e2, decoder, iters, device):
    test_domA, test_domB = get_test_imgs(args, device)
    exps = []
...
def get_test_imgs(args, device):
    ...
    domA_test= DomainAdaptationDataset(path_1='./custom_dataset/trainA', transforms=comp_transform)
    domB_test= DomainAdaptationDataset(path_1='./custom_dataset/trainB', transforms=comp_transform)
...
class DomainAdaptationDataset(data.Dataset):
    ...
    def __init__(self, path_1, transforms=None):
        super(DomainAdaptationDataset, self).__init__()

        self.path_1 = path_1
                    
        self.data_1 = [os.path.join(self.path_1, x) for x in os.listdir(self.path_1) if x.endswith(".png")]
        
        self.path_1_ = [x for x in os.listdir(self.path_1) if x.endswith(".png")]
    ...
    def __getitem__(self, index):
        ...
        _data1_name = self.path_1_[index]
